        ...
        if cat_cols:
            if cat_maps is None:
                ...
                for c in cat_cols:
                    ...
                    self.X_cat.append(X[c].map(mapping).fillna(0).astype(int).values)
                self.X_cat = torch.tensor(np.stack(self.X_cat, axis=1), dtype=torch.long)
            else:
                self.cat_maps = cat_maps
                self.X_cat = torch.tensor(
                    np.stack(
                        [X[c].map(self.cat_maps[c]).fillna(0).astype(int).values for c in cat_cols],
                        axis=1
                    ),
                    dtype=torch.long
                )
...
def run_ft_transformer(
    ...
    test_ds = TabularDataset(
        test_df, label_col, num_cols, cat_cols, task_type, train_ds.cat_maps
    )
